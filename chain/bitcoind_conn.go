...
func NewBitcoindConn(cfg *BitcoindConfig) (*BitcoindConn, error) {
	...
	bc.events, err = NewBitcoindEventSubscriber(
		cfg, client, bc.sendBlockToClients, bc.sendTxToClients,
	)
	if err != nil {
		return nil, err
	}
...
// sendBlockToClients is used to notify all rescan clients of a new block.
